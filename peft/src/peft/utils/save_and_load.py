...
def get_peft_model_state_dict(model, state_dict=None, adapter_name="default"):
    ...
    elif config.peft_type == PeftType.FLORA:
        bias = config.bias
        if bias == "none":
            to_return = {k: state_dict[k] for k in state_dict if "be_" in k}
        elif bias == "all":
            to_return = {k: state_dict[k] for k in state_dict if "be_" in k or "bias" in k}
        elif bias == "be_only":
            to_return = {}
            for k in state_dict:
                if "be_" in k:
                    to_return[k] = state_dict[k]
                    bias_name = k.split("be_")[0] + "bias"
                    if bias_name in state_dict:
                        to_return[bias_name] = state_dict[bias_name]
    else:
        raise NotImplementedError
